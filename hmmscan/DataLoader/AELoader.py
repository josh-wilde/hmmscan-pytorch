...
class AELoader:
    ...
    def __init__(self, directory, sequence_name, ae_type, lot_size_type, gap):
        # Set some initial metadata
        self.metadata = self.set_metadata(directory, sequence_name, ae_type, lot_size_type, gap)
        ...
        self.load_sequences(all_cols)

        # Add more metadata
        self.add_metadata()

    @staticmethod
    def set_metadata(directory, sequence_name, ae_type, lot_size_type, gap):
        metadata = {
            ...
            "lot_size_type": lot_size_type,  # type of lot size, name of column to get
        }
    ...
    def set_gap_fill(self, all_cols):
        """
        ...
        data = all_cols.sort_values(by=["lot_group", "num"]).rename(
            columns={self.metadata["ae_type"]: "ae", self.metadata["lot_size_type"]: "lot_size"}
        )
        data["lot_num_gap"] = data.groupby(["lot_group"])["num"].diff()
        data = data[["lot_num", "lot_num_gap", "lot_group", "prefix", "num", "ae", "lot_size"]].reset_index()
    ...
    def get_filler_lots(self):
        ...
        for index, row in lots_to_fill.iloc[1:, :].iterrows():
            if row["fill_before"]:
                ...
                current_fill_lot_nums = [
                    current_prefix + str(i).rjust(len_num_portion, "0") for i in current_fill_nums
                ]
        ...
        return pd.DataFrame(
            {"num": filled_nums, "prefix": filled_prefixes, "lot_num": filled_lot_nums, "lot_group": filled_lot_group}
        )
    ...
    def get_filled_df(self):
        ...
        filled = pd.merge(filled, self.raw_data[["lot_num", "ae", "lot_size"]], how="left", on="lot_num")

        # Make filler lots zero AE and mean lot size
        mean_lot_size = filled["lot_size"].mean(skipna=True)
        filled["ae"].fillna(0, inplace=True)
        filled["lot_size"].fillna(mean_lot_size, inplace=True)
    ...
    def split_sequences_single(self):
        """
        Input: filled lot dataframe (lot_num, ae, lot_size, lot_num_gap)
        Output: list of torch tensors that store sequences with 2 or more sequential lots
        """
            ...
            if index == self.filled_df.shape[0] - 1:  # if at last row of df
                if row["lot_num_gap"] == 1:
                    ...
                    sequences.append(
                        torch.cat(
                            [
                                torch.tensor(self.filled_df["ae"].iloc[seq_start_idx : index + 1]).reshape(-1, 1),
                                torch.tensor(self.filled_df["lot_size"].iloc[seq_start_idx : index + 1]).reshape(
                                    -1, 1
                                ),
                            ],
                            dim=1,
                    ...
                    sequences.append(
                        torch.cat(
                            [
                                torch.tensor(self.filled_df["ae"].iloc[seq_start_idx:index]).reshape(-1, 1),
                                torch.tensor(self.filled_df["lot_size"].iloc[seq_start_idx:index]).reshape(-1, 1),
                            ]
                        ),
                        dim=1,
                    )
                    sequences.append(
                        torch.cat(
                            [
                                torch.tensor(self.filled_df["ae"].iloc[index : index + 1]).reshape(-1, 1),
                                torch.tensor(self.filled_df["lot_size"].iloc[index : index + 1]).reshape(-1, 1),
                            ],
                            dim=1,
                        )
                    )
    ...
    def add_metadata(self):
        ...
        self.metadata["lot_size_mean"] = self.raw_data["lot_size"].mean()
        self.metadata["lot_size_q05"] = self.raw_data["lot_size"].quantile(0.05)
        self.metadata["lot_size_q25"] = self.raw_data["lot_size"].quantile(0.25)
        self.metadata["lot_size_q50"] = self.raw_data["lot_size"].quantile(0.5)
        self.metadata["lot_size_q75"] = self.raw_data["lot_size"].quantile(0.75)
        self.metadata["lot_size_q95"] = self.raw_data["lot_size"].quantile(0.95)
