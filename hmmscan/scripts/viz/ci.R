...
dfa_fits_fpath <- file.path(validation_dirpath, 'ci', 'fits', 'dfa_by_date_ex_iqr_outliers_serious_s3_c3.csv')
dfb_fits_fpath <- file.path(validation_dirpath, 'ci', 'fits', 'dfb_by_date_ex_iqr_outliers_serious_s3_c2.csv')
dfc_fits_fpath <- file.path(validation_dirpath, 'ci', 'fits', 'dfc_by_date_ex_iqr_outliers_serious_s2_c2.csv')
...
sample_params <- sample_fits %>% 
  ...
  filter(near(sum_wt, 1, tol = 0.0001)) %>% 
  ungroup() %>% 
  filter(state < n_states, component < n_mix_comps)
