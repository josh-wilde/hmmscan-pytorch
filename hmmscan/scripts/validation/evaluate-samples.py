...
def run_single_exp_sim_evaluator(
    ...
    model_ref: pd.DataFrame = pd.read_csv(os.path.join(get_ae_path("validation"), models_ref_fstub))
    ...
    models_to_fit: pd.DataFrame = models_to_fit.loc[
        (model_ref["n_states"] >= min_states)
        & (model_ref["n_states"] <= max_states)
        & (model_ref["n_mix_comps"] >= min_mix_comps)
        & (model_ref["n_mix_comps"] <= max_mix_comps),
        ["n_states", "n_mix_comps"],
    ]
    # Get the samples file
    experiment_samples: pd.DataFrame = pd.read_csv(
        os.path.join(get_ae_path("validation"), "simulation", "samples", f"eid_{eid:04d}.csv"), index_col=0
    )
    experiment_samples = experiment_samples.loc[
        (experiment_samples.index >= sample_index_min) & (experiment_samples.index <= sample_index_max), :
    ]
    ...
    for row_idx, row in work_to_do.iterrows():
        ...
        ae_sequence: torch.Tensor = torch.tensor(
            experiment_samples[experiment_samples.index == row["sample_id"]].values
        ).unsqueeze(1)
        lot_size_sequence: torch.Tensor = fixed_n * torch.ones_like(ae_sequence)
        scanner: Scanner = Scanner(
            ...
            lot_size_sequence=lot_size_sequence,
            verbose=False,
            max_iter=1000,
        )
        scanner_output: pd.DataFrame = scanner.run(predict=True)
...
def run_use_case_evaluator(
    ...
    models_to_fit: pd.DataFrame = pd.DataFrame({"n_states": range(1, max_states + 1)}).join(
        pd.DataFrame({"n_mix_comps": range(1, max_mix_comps + 1)}), how="cross"
    )
    ...
    experiment_samples: pd.DataFrame = pd.read_csv(
        os.path.join(get_ae_path("validation"), "use_case", "samples", f"{generating_model_spec}.csv"), index_col=0
    )
    ...
    if os.path.exists(output_fpath):
        ...
        eval_df: pd.DataFrame = existing_fits_df.loc[
        ...
        if eval_bic > min_bic:
            ...
            # Get rid of the stuff that already exists
            work_to_do = (
                pd.merge(work_to_do, existing_fits, on=["n_states", "n_mix_comps"], indicator=True, how="left")
                .query('_merge=="left_only"')
                .drop("_merge", axis=1)
            )
    else:
    ...
    if len(work_to_do) > 0:
        work_to_do["temp_is_eval"] = (work_to_do["n_states"] == eval_n_states) & (
            work_to_do["n_mix_comps"] == eval_n_mix_comps
        )
    ...
    for row_idx, row in work_to_do.iterrows():
        ...
        scanner_output: pd.DataFrame = scanner.run(predict=False)
        best_bic_init: pd.Series = scanner_output.sort_values(by="bic", ascending=True).iloc[0]
...
def run_ci_evaluator(
    ...
    init_row_df: pd.DataFrame = init_df.loc[
        ...
        & (init_df["lot_size_type"] == generating_lot_size_type)
    ...
    # Get the lot size sequence
    data_df = load_data(
        f"gap_filled/{generating_ae_type}_{generating_lot_size_type}/gap_1/", f"{generating_sequence_name}.csv"
    )
    lot_size_sequence: torch.Tensor = torch.tensor(data_df["lot_size"].values, dtype=torch.int)
    ...
    if os.path.exists(output_fpath):
        existing_fits_df: pd.DataFrame = pd.read_csv(output_fpath, index_col=0)
        existing_fits: pd.DataFrame = existing_fits_df[
    ...
    for row_idx, row in work_to_do.iterrows():
        ...
        dl = ManualLoader(
            ...
            sequence_name=f"{generating_model_spec}_sample{row['sample_id']}",
            ae_type="sample",
        )

        # Get the fits from 25 random initializations
        output = {}
        for rseed in [2702 + i for i in range(25)]:
            # Initialize the model
            model = initialize_model(row["n_states"], row["n_mix_comps"], 1000 * rseed)

            # Fit the model
            model.fit_loader(dl)

            # Get predictions
            state_predictions: torch.Tensor = model.predict_states(dl.sequences)

            # Output dict
            output: dict = report_model_dl_data(model, dl, state_predictions, output)

        scanner_output: pd.DataFrame = convert_output_dict_to_df(output, max_s=4, max_c=9)
        best_bic_init: pd.Series = scanner_output.sort_values(by="bic", ascending=True).iloc[0]
        best_bic_init["sample_id"] = row["sample_id"]
        best_bic_init["generating_model_spec"] = generating_model_spec
        fits_output_series_dict[row_idx] = best_bic_init
...
def main():
    ...
    parser.add_argument("--generating_ae_type", type=str, default="serious")
    parser.add_argument("--generating_lot_size_type", type=str, default="lot_size_doses")
