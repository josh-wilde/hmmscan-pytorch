...
def get_twostate_transition_probs(params: dict[str, Any]) -> dict[str, float]:
    ...
    return {"s0_stat": s0_stat, "s1_stat": s1_stat, "s0_s0": p00, "s0_s1": p01, "s1_s0": p10, "s1_s1": p11}
...
def get_multiplestate_ssd_params(
    ...
    if n_mix_comps == 1:
        ...
        ovl_inter_diff: float = (
            ovl_inter_results["x"] - s0_0_param
        )  # this is diff in p_high and p_low that induces ovl
...
def ovl_error(x: np.ndarray, n: int, p_low: float, ovl: float) -> float:
    return (ovl - sum(min(binom.pmf(k=k, n=n, p=p_low), binom.pmf(k=k, n=n, p=x[0])) for k in range(n + 1))) ** 2
...
def solve_ovl(n: float, p_low: float, p_comp_diff: float, ub: float, ovl: float) -> dict[str, float]:
    if p_comp_diff == 0:
        opt_result: OptimizeResult = minimize(
            fun=ovl_error, x0=np.array([p_low]), bounds=[(p_low, ub)], args=(n, p_low, ovl)
        )
    else:
        opt_result: OptimizeResult = minimize(
...
def get_simulation_inputs(sim_dirpath: str, input_fpath: str, output_fpath: str) -> pd.DataFrame:
    ...
    new_output: pd.DataFrame = raw_inputs.merge(
        tm_output_unique,
        how="left",
        on=[
            "n_states",
